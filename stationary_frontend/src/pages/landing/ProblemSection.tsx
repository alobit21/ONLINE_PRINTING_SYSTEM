...
export const ProblemSection = () => {
    ...
    return (
        <section id="problems" className="section-padding bg-gray-800 relative overflow-hidden">
            <div className="section-container">
                <div className="heading-centered">
                    <h2 className="text-brand-600 font-black tracking-widest uppercase text-xs mb-4">The Challenge</h2>
                    <h3 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-8 leading-tight">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {problems.map((problem, index) => (
                        <motion.div
                            ...
                            className="p-10 rounded-[2.5rem] bg-gray-700 border border-gray-600 shadow-sm hover:shadow-2xl transition-all duration-500 group"
                        >
                            <div className={`${problem.color} p-4 rounded-2xl w-fit mb-8 group-hover:scale-110 transition-transform`}>
                                {problem.icon}
                            </div>
                            <h4 className="text-2xl font-black text-white mb-4">{problem.title}</h4>
                            <p className="text-gray-300 leading-relaxed font-medium">{problem.description}</p>
                        </motion.div>
                    ))}
                </div>
            </div>
