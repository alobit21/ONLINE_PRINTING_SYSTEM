...
export const GuestShopSelection = () => {
    ...
    return (
      <div className="max-w-5xl mx-auto p-4">
        <div className="bg-red-100 border border-red-300 p-4 rounded text-center">
          <p className="text-red-800">Error loading shops: {error.message}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-5xl mx-auto p-4">
        <div className="bg-yellow-100 border border-yellow-300 p-4 rounded text-center">
          <p className="text-yellow-800">No shops are currently accepting orders.</p>
          <p className="text-yellow-700 mt-2">Please try again later.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Choose a Printing Shop</h2>
        <p className="text-gray-600">Select a shop to handle your printing order</p>
      </div>
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="font-semibold text-lg text-gray-900 mb-1">{shop.name}</h3>
              ...
              <div className="flex items-center justify-between">
                <div className={`flex items-center text-sm ${
                  shop.isAcceptingOrders ? 'text-green-600' : 'text-gray-400'
                }`}>
                  <Clock className="w-4 h-4 mr-1" />
                  {shop.isAcceptingOrders ? 'Accepting orders' : 'Not accepting'}
                </div>
                {shop.subscriptionTier && (
                  <div className="text-xs bg-brand-100 text-brand-700 px-2 py-1 rounded">
                    {shop.subscriptionTier}
                  </div>
                )}
              </div>
      ...
      {selectedShop && (
        <div className="mt-8 flex justify-center">
          <button 
            ...
            className="bg-brand-600 text-white hover:bg-brand-700 px-8 py-3 rounded-lg font-medium transition-colors"
          >
